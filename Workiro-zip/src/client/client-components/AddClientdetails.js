...
let folderData = [];
const AddClientdetails = React.memo(
  ({
    userDetail,
    setUserDetail,
    setDataCompanyHouse,
        ...
        webClientCLS.Json_GetConfiguration(requestBody, (sts, data) => {
          if (sts) {
            if (data) {
              ...
              try {
                const defaultStatus = json.Table.find(
        ...
        Cls.Json_GetFolders(requestBody, (sts, data) => {
          if (sts) {
            if (data) {
              ...
              const sourceObj = folderData.find(
                (item) => item.FolderID == localStorage.getItem("FolderId")
              );
              ...
              if (sourceObj) {
                ...
                console.error(
                  "Folder ID not found in folderData:",
    ...
    const Json_CompanyHouseDetails = (inputValue) => {
      let requestBody = {
    ...
    const Json_CompanyDetails = (inputValue) => {
      let requestBody = {
        CompanyName_Number: inputValue,
      };
        ...
        Cls.Json_CompanyHouseDetails(requestBody, (sts, data) => {
          if (sts) {
            ...
            if (data) {
              ...
              toast.error(
                "Unable to fetch Company Details. Failed to receive information from Companies House."
              );
            }
          } else {
